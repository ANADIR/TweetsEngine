...
class TwitterOAuthClass: NSObject {
    ...
    func setTwitterKeys(AppConsumerKey:String,AppConsumerSecret:String){
        self.consumerKey = AppConsumerKey
        self.consumerSecret = AppConsumerSecret
    }
    ...
    public func searchTweets(searchString: String, completionBlock:@escaping (NSMutableArray) -> ()) {
        if(isAuthenticated) {
            do {
